...
CREATE DEFINER=`barath`@`%` PROCEDURE `GET_OBJECT`(INOUT P_OBJECT_ID NUMERIC,
                               ...
                               OUT P_OBJECT_XPATH VARCHAR(500),
...
CREATE DEFINER=`barath`@`%` PROCEDURE `GET_TEST_SUITE`(INOUT P_TEST_SUITE_ID NUMERIC,
                                   ...
                                   OUT P_APPLICATION_ID NUMERIC)
BEGIN
SELECT TEST_SUITE_ID,TEST_SUITE_NAME,NEW_BROWSER,APPLICATION_ID
INTO P_TEST_SUITE_ID,P_TEST_SUITE_NAME,P_NEED_NEW_BROWSER,P_APPLICATION_ID
FROM QA_TEST_SUITE
WHERE TEST_SUITE_ID = P_TEST_SUITE_ID;
END$$
DELIMITER ;
...
(DEP_ID,DATA_KEY,DATA_VALUE,FLOW_ID,CREATED_DT,step_id)
VALUES
(P_DEP_ID, P_REF_KEY,P_CAPTURED_VALUE,P_REF_FLOW_ID,now(),P_REF_STEP_ID);
COMMIT;
END$$
DELIMITER ;
